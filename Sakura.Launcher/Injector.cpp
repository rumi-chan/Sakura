	...
	if (LI_FN(K32EnumProcessModules)(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
		for (auto i{ 0u }; i < (cbNeeded / sizeof(HMODULE)); ++i) {
			...
			if (LI_FN(K32GetModuleBaseNameW)(hProcess, hMods[i], szModName, sizeof(szModName) / sizeof(TCHAR))) {
				if (std::wcscmp(szModName, _XorStrW(L"Sakura.dll")) == 0) {
...
bool WINAPI Injector::inject(const std::uint32_t pid) noexcept
{
	...
	const auto dll_path{ std::wstring(current_dir) + _XorStrW(L"\\Sakura.dll") };

	if (const auto f{ std::ifstream(dll_path) }; !f.is_open()) {
		LI_FN(MessageBoxW)(nullptr, _XorStrW(L"Sakura.dll file could not be found.\nTry reinstalling the cheat."), _XorStrW(L"Sakura"), MB_ICONERROR | MB_OK);
...
void Injector::autoUpdate()
{
	...
	try
	{
		auto json = msclr::interop::marshal_as<std::string>(client->DownloadString(xor_clrstr_w(L"https://api.github.com/repos/rumi-chan/Sakura/releases/latest")));
		...
		if (std::smatch tagnameMatch; std::regex_search(json, tagnameMatch, tagnameRegex))
		{
			...
			{
				if (!System::IO::File::Exists(xor_clrstr_w(L"Sakura.dll")))
				{
					throw gcnew Exception(xor_clrstr_w(L"Failed to find Sakura.dll in the current directory"));
				}
				...
				const auto date_of_current_release = System::IO::File::GetLastWriteTime(xor_clrstr_w(L"Sakura.dll")).ToString(xor_clrstr_w(L"dd.MM.yyyy HH:00"));
				if (date_of_current_release != date_of_github_release)
				{
					...
					const auto result = MessageBox::Show(xor_clrstr_w(L"New version is available on GitHub\nWould you like to download it now?"), xor_clrstr_w(L"Sakura"), MessageBoxButtons::YesNo, MessageBoxIcon::Information);
					if (result == DialogResult::Yes)
					{
						if (std::smatch urlMatch; std::regex_search(json, urlMatch, urlRegex))
						{
							auto url = gcnew String(urlMatch[1].str().c_str());
							auto file = String::Format(xor_clrstr_w(L"Sakura_{0}.zip"), version);
							client->DownloadFile(url, file);

							//System::IO::Compression::ZipFile::ExtractToDirectory(file, xor_clrstr_w(L"Sakura"));
							//System::IO::File::Delete(file);
							//System::IO::File::Delete(xor_clrstr_w(L"Sakura.dll"));
							//System::IO::File::Move(xor_clrstr_w(L"Sakura\\Sakura.Launcher.exe"), String::Format(xor_clrstr_w(L"Sakura.Launcher_{0}.exe"), version));
							//System::IO::File::Move(xor_clrstr_w(L"Sakura\\Sakura.dll"), xor_clrstr_w(L"Sakura.dll"));
							//System::IO::Directory::Delete(xor_clrstr_w(L"Sakura"));
							//
							...
							//System::Diagnostics::Process::Start(String::Format(xor_clrstr_w(L"Sakura.Launcher_{0}.exe"), version));
	...
	{
		MessageBox::Show(String::Format(xor_clrstr_w(L"{0} - {1}"), e->Message, e->StackTrace->Substring(5)), xor_clrstr_w(L"Sakura"), MessageBoxButtons::OK, MessageBoxIcon::Error);
		Environment::Exit(0);
	}
}
...
void Injector::run() noexcept
{
	...
	while (true) {
		...
		SakuraLauncher::gameState = (league_processes.size() > 0) ? true : false;
		SakuraLauncher::clientState = (league_client_processes.size() > 0) ? true : false;
		...
		for (auto& pid : league_processes) {
			if (!Injector::isInjected(pid)) {
				SakuraLauncher::cheatState = false;
				if (SakuraLauncher::btnState) {
					std::this_thread::sleep_for(1.5s);
					if (Injector::inject(pid))
						SakuraLauncher::cheatState = true;
					else
						SakuraLauncher::cheatState = false;
				}
