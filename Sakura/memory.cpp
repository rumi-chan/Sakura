...
[[nodiscard]] static std::uint8_t* find_signature(const wchar_t* szModule, const char* szSignature) noexcept
{
	...
	std::uint8_t* next_check_address{ nullptr };
...
void Memory::Search(bool gameClient)
{
	...
	try {
		...
		while (true) {
			...
			for (auto& sig : signatureToSearch) {
				...
				for (auto& pattern : sig.pattern) {
					...
					if (!address) {
						::MessageBoxA(nullptr, ("Failed to find pattern: " + pattern).c_str(), "Sakura", MB_OK | MB_ICONWARNING);
						continue;
					}
		...
		::MessageBoxA(nullptr, e.what(), "Sakura", MB_OK | MB_ICONWARNING);
	}
}
